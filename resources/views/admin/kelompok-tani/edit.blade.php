@extends('layouts.app')
@section('header_title', 'Edit Kelompok Tani')
@section('header_subtitle', 'Perbarui data kelompok tani.')
...
@section('content')
    <div style="max-width:800px; margin:0 auto;">
        ...
        <form action="{{ route('admin.kelompok-tani.update', $kelompokTani) }}" method="POST" style="margin-top:20px;">
            @csrf @method('PUT')
            <div class="card" style="padding:24px; background:#fff; border-radius:16px; box-shadow:0 4px 15px rgba(0,0,0,0.04); border:1px solid #eef2f0;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:16px;">
                    <div>
                        ...
                        <input type="text" name="nama_kelompok" value="{{ old('nama_kelompok', $kelompokTani->nama_kelompok) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit; font-weight:600;">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Nama Ketua</label>
                        <input type="text" name="ketua" value="{{ old('ketua', $kelompokTani->ketua) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:14px; margin-bottom:16px;">
                    <div>
                        ...
                        <input type="text" name="jorong" value="{{ old('jorong', $kelompokTani->jorong) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Jumlah Anggota</label>
                        <input type="number" name="jumlah_anggota" value="{{ old('jumlah_anggota', $kelompokTani->jumlah_anggota) }}" min="1" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Luas Lahan</label>
                        <input type="text" name="luas_lahan" value="{{ old('luas_lahan', $kelompokTani->luas_lahan) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:14px; margin-bottom:16px;">
                    <div>
                        ...
                        <input type="text" name="komoditas_utama" value="{{ old('komoditas_utama', $kelompokTani->komoditas_utama) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Produktivitas</label>
                        <input type="text" name="produktivitas" value="{{ old('produktivitas', $kelompokTani->produktivitas) }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                    <div>
                        ...
                        <select name="status" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                            ...
                            <option value="Non-Aktif" {{ old('status', $kelompokTani->status) == 'Non-Aktif' ? 'selected' : '' }}>Non-Aktif</option>
                        </select>
                    </div>
                </div>
