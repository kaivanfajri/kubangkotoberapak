@extends('layouts.app')
@section('header_title', 'Tambah UMKM Baru')
@section('header_subtitle', 'Daftarkan usaha mikro baru ke katalog digital nagari.')
...
@section('content')
    <div style="max-width:800px; margin:0 auto;">
        ...
        <form action="{{ route('admin.umkm.store') }}" method="POST" enctype="multipart/form-data" style="margin-top:20px;">
            @csrf
            <div class="card" style="padding:24px; background:#fff; border-radius:16px; box-shadow:0 4px 15px rgba(0,0,0,0.04); border:1px solid #eef2f0;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:16px;">
                    <div>
                        ...
                        <input type="text" name="nama_usaha" value="{{ old('nama_usaha') }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit; font-weight:600;">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Pemilik</label>
                        <input type="text" name="pemilik" value="{{ old('pemilik') }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:14px; margin-bottom:16px;">
                    ...
                    <div>
                        ...
                        <input type="text" name="jam_operasional" value="{{ old('jam_operasional') }}" placeholder="08:00 - 17:00 WIB" style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                    </div>
                </div>

                <div style="margin-bottom:16px;">
                    <label style="font-size:12px; font-weight:700; color:var(--muted); display:block; margin-bottom:4px;">Alamat / Jorong</label>
                    <input type="text" name="alamat" value="{{ old('alamat') }}" required style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                </div>
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="produk_utama" value="{{ old('produk_utama') }}" placeholder="Rendang Daging, Kalio Daging, Serundeng" style="width:100%; border:1.5px solid #e0e6e0; border-radius:10px; padding:10px 14px; font-family:inherit;">
                </div>
